...
// 224 // /为了限制网络流量，我们重新发布:
// 225 // 1)只有当它们是一个正在积极解决的块的非重播投票。沉降块受阻塞PoW的限制
// 226 // 2)代表有一个权值> Y，以防止创建许多小权帐户来发送选票
// 227 // 3)只有在前一X秒内没有收到该代表对该分组的表决时才可这样做。
// 228 /这阻止了随序列号增加而迅速公布选票。
// 229 / /
// //这些规则是由调用者实现的，而不是这个函数。
...
public:
	...
	void confirm_ack (rai::confirm_ack const & message_a) override//广播？？？
	{
		...
		if (vote.code == rai::vote_code::replay)
		{
			...
			// //这试图通过回放我们已知的最高投票给代表节点来帮助那些已经丢失了最高序列号的代表节点
			// 只有当序列号与网络重新排序的原因显著不同时才这样做
			// //放大攻击注意事项:我们发送一个confirm_ack来响应一个confirm_ack，但没有网络流量增加
...
// 使用sqrt_broadcast_simulation进行模拟，我们只需要向sqrt(total_peer)随机对等点广播，就可以成功地向每个人发布高概率的内容
...

	{
		...
		for (; i != n && announcements < announcements_per_interval; ++i)
		{
			...
			else
			{
				...
				// 如果超过一个完整的公告间隔已经通过，并且没有人对该块进行投票，我们需要同步
		...
		// //标记余数为发送的0个公告
		// 如果有大量的分叉，网络将会在增加根哈希顺序的情况下解决这些问题。
		// 这是一个DoS保护机制，用于限制解决fork的通信量。
		for (; i != n; ++i)
		{
...
void rai::port_mapping::refresh_mapping ()
{
	if (rai::rai_network != rai::rai_networks::rai_test_network)
	{
		...
		// 故意省略:我们不映射RPC端口，因为除非添加RPC身份验证，否则这几乎总是安全风险
